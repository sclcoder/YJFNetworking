...
void YJFNetLog(NSString *format, ...) {
#ifdef DEBUG
    if (![YJFNetworkConfig shareConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
@implementation YJFNetworkUtils
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        YJFNetLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation YJFBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<YJFRequestAccessory> accessory in self.requestAccessories) {
...
@implementation YJFBatchRequest (RequestAccessory)
...
@implementation YJFChainRequest (RequestAccessory)
